...
def train_and_eval(MuAc_model, train_loader, val_loader, optimizer, criterion, epochs):
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    MuAc_model.to(device)
    ...
    for epoch in range(epochs):
        ...
        for inputs,labels in tqdm(train_loader, desc = f' Epochs {epoch+1}/{epochs} - Training'):
            ...
            # Core training steps
            optimizer.zero_grad()
            main_output,aux_output = MuAc_model(inputs)
        ...
        if val_accuracy > best_val_acc:
            ...
            best_model_weights = copy.deepcopy(MuAc_model.state_dict())
    
    # Load best model
    MuAc_model.state_dict(best_model_weights)
...
all_files = [item[0] for item in MuAc_dataset.data]
all_labels = [item[1] for item in MuAc_dataset.data]
...
# Dataloaders
loader = DataLoader(MuAc_dataset, batch_size = 32, shuffle = True)
...
for fold, (train_index, val_index) in enumerate(skf.split(all_files,all_labels)):
    train_dataset = torch.utils.data.Subset(MuAc_dataset, train_index)
    val_dataset = torch.utils.data.Subset(MuAc_dataset, val_index)
    ...
    for epochs in epochs_range:
        for lr in learning_rates:
            ...
            # Load model
            MuAc_model = models.inception_v3(pretrained=True)
            
            # Modify for transfer learning
            num_ftrs = MuAc_model.fc.in_features
            MuAc_model.fc = nn.Linear(num_ftrs,3)
            
            # Add Stochastic Gradient Descent with Momentum 0.9
            optimizer = optim.SGD(MuAc_model.parameters(), lr=lr, momentum=0.9)
            
            # Run the big training and evaluation function
            gt_labels, predictions, accuracy = train_and_eval(MuAc_model, train_loader, val_loader, optimizer, criterion, epochs)
            
            # Save model
            model_filename = f'{save_dir}/fold{fold}_epoch_{epochs}_lr{lr}_model.pth'
            torch.save(MuAc_model, model_filename)
...
results_df.to_excel('C:\\Users\\PATH\\MuAc_training_results.xlsx', index=False)
